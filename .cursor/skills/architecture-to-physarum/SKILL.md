...
Current status: Skill 1 is complete for this interface. All 15 catalog archetypes translate into Physarum behavior. Catalog ratings and descriptors in `lib/catalog.ts` remain locked source of truth. The official UI is `LivingInstrument` on `app/page.tsx`. Translation, topology recipes, and the agent/trail engine live in `lib/skill1/`. The Skill 2 payload is `Skill1Handoff` from `toHandoff()`. Do not generate evaluated 2D sections, vertical propagation, or 3D geometry here — that work belongs to Skill 2 and Skill 3.
## Purpose
...
→ PHYSARUM PARAMETERS
→ EMERGENT NETWORK (skeleton)
→ ARCHITECTURAL TOPOLOGY (occupancy → mass / void / enclosure)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
HANDOFF TO SKILL 2
```
The Physarum network is generative intelligence, not the final architecture. Void Field organizes mass around a central absence. Contained Room concentrates an interior volume inside a larger mass. Do not hard-code those shapes; they emerge from occupancy fields warped by trails, attraction, and permeability.
...
Concrete handoff in this repository:
- Type: `Skill1Handoff` in `lib/skill1/types.ts`
- Builder: `toHandoff(translateArchetype(archetypeId))` in `lib/skill1/translate.ts`
- Payload includes typology, archetype, locked ratings, descriptors, traces, rankings, `params`, `recipe`, `behavior`, topology kind, and field source/attractor
- Official board: `components/living-instrument.tsx` (2D 20×20 plan field, locked criteria sliders, Generate / Reset / Regenerate)
- Criteria ranks cannot be edited; they come from `lib/catalog.ts`
Until a later Skill 1 change is requested, treat this translation as frozen for Skill 2.
## This Skill Must Not
